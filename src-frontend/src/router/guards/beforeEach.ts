...
async function handleRouteGuard(
  ...
  if (!routeRegistry?.isRegistered() && userStore.isLogin) {
    ...
    const targetRouteExists = to.name && existingRouteNames.has(to.name as string)

    if (menuHasRoutes && dynamicRoutesExist && targetRouteExists) {
...
async function handleDynamicRoutes(
  ...
  try {
    ...
    console.log('[RouteGuard] 菜单数据获取成功:', menuList.length, '个菜单')
    ...
    console.log('[RouteGuard] 动态路由注册完成')
    ...
    console.log('[RouteGuard] homePath:', homePath.value)
    const { path: validatedPath, hasPermission } = RoutePermissionValidator.validatePath(
      to.path,
      menuList,
      homePath.value || '/'
    )
    console.log('[RouteGuard] 路径验证结果:', { toPath: to.path, validatedPath, hasPermission })

    // 初始化成功，重置进行中标记
    routeInitInProgress = false
    closeLoading()
    console.log('[RouteGuard] 路由初始化完成，准备导航到:', to.path)
    ...
    if (isHttpError(error)) {
      console.error(`[RouteGuard] HttpError - code: ${error.code}, message: ${error.message}`)
    }
