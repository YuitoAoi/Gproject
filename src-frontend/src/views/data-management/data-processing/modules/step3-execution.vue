                ...
                <div class="funnel-chart">
                  ...
                  <div class="funnel-arrow">
                    ...
                    <span class="text-xs text-g-500"
                      >— {{ formatNumber(task.filteredCount) }} 条</span
                    >
                  </div>
                  ...
                  <div class="funnel-row">
                    <div class="funnel-bar" :style="{ width: filterRatio + '%' }">
                      ...
                      <span class="funnel-bar__count"
                        >{{ formatNumber(task.rawCount - task.filteredCount) }} 条</span
                      >
                    </div>
                  </div>
                  ...
                  <div class="funnel-arrow">
                    <span class="ri:arrow-down-line text-g-400"></span>
                    <span class="text-xs text-g-500"
                      >— {{ formatNumber(task.dedupedCount) }} 条</span
                    >
                  </div>
                  ...
                  <div class="funnel-row">
                    <div class="funnel-bar" :style="{ width: dedupRatio + '%' }">
                      ...
                      <span class="funnel-bar__count"
                        >{{
                          formatNumber(task.rawCount - task.filteredCount - task.dedupedCount)
                        }}
                        条</span
                      >
                    </div>
                ...
                <div class="funnel-result">
                  ...
                  <div>
                    ...
                    <div class="text-xl font-bold text-success"
                      >{{ formatNumber(task.finalCount) }} 条</div
                    >
                    <div class="text-xs text-g-400"
                      >产出率: {{ ((task.finalCount / task.rawCount) * 100).toFixed(1) }}%</div
                    >
                  </div>
                </div>
              </div>
...
<script setup lang="ts">
  ...
  const filterRatio = computed(() => {
    ...
    return Math.max(
      ((props.task.rawCount - props.task.filteredCount) / props.task.rawCount) * 100,
      20
    )
  })
  ...
  const dedupRatio = computed(() => {
    if (!props.task || !props.task.rawCount) return 100
    return Math.max(
      ((props.task.rawCount - props.task.filteredCount - props.task.dedupedCount) /
        props.task.rawCount) *
        100,
      20
    )
