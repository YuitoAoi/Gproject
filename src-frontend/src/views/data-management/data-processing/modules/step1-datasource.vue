    ...
    <div v-if="filteredDatasets.length > 0" class="dataset-grid">
      <div
        ...
        :class="{
          ...
          'is-disabled': ds.status === 'disabled'
        }"
      ...
      >
        ...
        <div class="dataset-card__radio">
          <span
            :class="selectedId === ds.id ? 'ri:radio-button-line text-xl' : 'ri:checkbox-blank-circle-line text-xl'"
            :style="{ color: selectedId === ds.id ? 'var(--el-color-primary)' : 'var(--art-gray-300)' }"
          ></span>
        </div>
        ...
        <div class="dataset-card__body">
          <div class="flex items-center gap-2 mb-2">
            ...
            <ArtSvgIcon
              ...
              class="text-sm text-g-400 cursor-help"
              v-tooltip="`衍生自：${ds.lineage.sourceName} — 应用规则：${ds.lineage.rules.join('、')}`"
            />
          </div>
          <div class="flex items-center gap-2 text-xs text-g-500 mb-3">
            <span>大小: {{ formatSize(ds.size) }}</span>
            <span class="text-g-300">|</span>
            <span>状态: </span>
            <ElTag :type="statusType(ds.status)" size="small" effect="plain">{{ statusLabel(ds.status) }}</ElTag>
          </div>
          <div class="flex flex-wrap gap-1.5" v-if="ds.tags?.length">
            <span
              v-for="tag in ds.tags"
              :key="tag.label"
              class="dataset-card__tag"
...
const filteredDatasets = computed(() => {
  let list = props.datasets.filter((d) => d.status !== 'disabled')
  ...
  if (filterFormat.value) {
    list = list.filter((d) => d.format === filterFormat.value)
  }
...
function handleSelect(ds: DatasetItem) {
  if (ds.status === 'disabled') return
  emit('select', ds.id)
}
...
function statusType(status: string): 'primary' | 'success' | 'info' | 'warning' | 'danger' {
  const map: Record<string, 'primary' | 'success' | 'info' | 'warning' | 'danger'> = { ready: 'success', processing: 'warning', error: 'danger', disabled: 'info' }
  return map[status] || 'info'
}

function statusLabel(status: string): string {
  const map: Record<string, string> = { ready: '已就绪', processing: '待清洗', error: '异常', disabled: '禁用' }
...
.dataset-card {
  ...
  &.is-selected {
    ...
    background: linear-gradient(135deg, rgba(var(--el-color-primary-rgb, 64, 158, 255), 0.03) 0%, #fff 100%);
  }
