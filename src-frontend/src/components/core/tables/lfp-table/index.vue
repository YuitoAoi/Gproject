...
<template>
  <div class="lfp-table" :class="{ 'is-empty': isEmpty }" :style="containerHeight">
...
<script setup lang="ts">
  ...
  /** LfpTable 组件的 Props 接口 */
  interface LfpTableProps extends TableProps<Record<string, any>> {
    /** 加载状态 */
    loading?: boolean
    /** 列渲染配置 */
    ...
    /** 空数据时显示的文本 */
    emptyText?: string
    /** 是否开启 LfpTableHeader，解决表格高度自适应问题 */
    showTableHeader?: boolean
  }

  const props = withDefaults(defineProps<LfpTableProps>(), {
    columns: () => [],
    fit: true,
    showHeader: true,
    stripe: undefined,
  ...
  const mergedTableProps = computed(() => ({
    ...
    // Element Plus 默认值为 true，未显式传入时不应被 LfpTable 覆盖成 false。
  ...
  const findTableHeader = () => {
    ...
    const tableHeader = document.getElementById('lfp-table-header')
    if (tableHeader) {
