...
def execute_pipeline(job_id: str, params: GenerateRequest, input_path: str):
    ...
    manager.update(job_id, status="running", started_at=now, progress=0.0, stage="准备中")
    ...
    # ── run pipeline ─────────────────────────────────────────────────────
    manager.update(job_id, progress=0.05, stage="环境就绪")
    engine = None
    try:
        ...
        manager.update(job_id, progress=0.85, stage="数据生成完成")

        row_count = _collect_and_save(results, temp_dir, output_path)

        manager.update(job_id, progress=0.95, stage="结果保存")
